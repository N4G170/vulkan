...
void main()
{
	...
	float shade = 1.0;
	shade = diffuse_factor < 0.5 ? 0.75 : shade;
	shade = diffuse_factor < 0.35 ? 0.6 : shade;
	shade = diffuse_factor < 0.25 ? 0.5 : shade;
	shade = diffuse_factor < 0.1 ? 0.25 : shade;

	out_colour = vec4( colour * texture(texture_sampler, in_uv).rgb * shade , 1.0);
}
